//! Validation of one durable mining job against live cross-owner state.
...
use crate::mining::{MiningJobId, MiningJobRecord};

mod references;

use references::{MiningJobReferences, resolve_mining_job_references};
...
pub(super) fn validate_mining_equipment_portability(
    job: MiningJobId,
    definition: &crate::equipment::EquipmentDefinition,
) -> Result<(), MiningJobValidationError> {
...
pub(super) fn validate_loaded_mining_job(
    registries: &Registries,
    state: &AppState,
    job: &MiningJobRecord,
) -> Result<(), MiningJobValidationError> {
