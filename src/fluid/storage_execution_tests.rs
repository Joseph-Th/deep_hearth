...
fn commit_transfer(
    ...
    let resolution =
        make_test_fluid_transfer_resolution(source, destination, Volume::from_microliters(volume));
...
fn incompatible_identity_or_temperature_requires_a_separate_mixing_resolver() {
    ...
    assert!(matches!(
        validate_fluid_transfer(
            &registries,
            &state,
            make_test_fluid_transfer_resolution(source, other_fluid, Volume::from_microliters(1),),
        ),
...
fn capacity_failure_and_stale_token_leave_fluid_unchanged() {
    ...
    assert!(matches!(
        validate_fluid_transfer(
            &registries,
            &state,
            make_test_fluid_transfer_resolution(source, destination, Volume::from_microliters(6),),
        ),
