...
fn validate_element_embodied_trace(
    ...
    if !materials.has_commodity(commodity) {
        return Err(StructureValidationError::UnknownEmbodiedCommodity { element: record.id });
    }
    ...
    .map_err(
        |error| StructureValidationError::InvalidEmbodiedParticleSizeState {
            element: record.id,
            error,
        },
    )?;
...
fn validate_support_index_coverage(state: &StructureState) -> Result<(), StructureValidationError> {
    for id in state
        .supports_by_element
        .keys()
...
fn validate_support_edges(state: &StructureState) -> Result<(), StructureValidationError> {
...
fn validate_reverse_support_edges(state: &StructureState) -> Result<(), StructureValidationError> {
...
fn validate_active_supports(state: &StructureState) -> Result<(), StructureValidationError> {
