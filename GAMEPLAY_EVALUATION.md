...
| report `fieldwork` | Sample-then-extract mining orders with wear-aware tool choice, projected versus executed cost, partial-order and supply-stop accounting, indexed-survey information payoff, same-site exploitation, and new-site recovery. Known-site repeats carry tool condition and remaining represented reserve. If the initial site cannot satisfy the order, the actual partial state continues through the three already-admitted follow-up sites using the same carried mining tool/survival state and canonical prospecting/mining. Before that bounded multi-site search, the actor prices the copper sampling-hammer upgrade against the disclosed three-site horizon and uses indexed surveying only when the same pre-action return policy justifies it. A matched point-search branch measures realized information payoff only after the choice is frozen, so an ex-post loss remains evidence about uncertainty rather than an oracle for reselection. The report preserves total fulfillment and any demand still left after the bounded local search area is exhausted; later known-site depletion similarly proves productive rerouting without manufacturing replacement kit. Policy uses acquired evidence only: a fully localized physical sample may provide a coarse conservative resource-mass band that caps tool investment, while exact reserve remains hidden and realized depletion arrives through committed claims/target refresh. Maintained ordinary cases execute both sides of quarry investment at both relevant hardness tiers: a sufficiently large visible soft-rock reserve selects the stone quarry tool; a sufficiently large medium-hardness reserve selects the copper-reinforced quarry; and a nominal medium-hardness bulk order with a small localized reserve is cut back before construction and keeps the lighter reinforced pick. Exploratory fieldwork varies player demand independently across short, current-project, and settlement-scale bulk horizons while primary and follow-up hidden reserves independently span shallow, common, and bulk opportunity classes; bounded seed contracts require all nine primary demand/reserve combinations and heterogeneous follow-up reserves so evaluator setup cannot manufacture tool payback or recovery by coupling a large goal to a rich site. |
