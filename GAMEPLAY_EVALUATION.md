...
| report `fieldwork` | Sample-then-extract mining orders with wear-aware tool choice, projected versus executed cost, partial-order and supply-stop accounting, indexed-survey information payoff, same-site exploitation, and new-site recovery. Known-site repeats carry tool condition and remaining represented reserve; if the initial order or later exploitation depletes the site, the actual partial/depleted state reuses the field kit to survey another site and resume extraction. Policy uses acquired evidence only: a fully localized physical sample may provide a coarse conservative resource-mass band that caps tool investment, while exact reserve remains hidden and realized depletion arrives through committed claims/target refresh. Maintained ordinary cases execute both sides of the quarry-investment decision: a sufficiently large visible soft-rock reserve selects the heavy quarry tool, while a nominal bulk order with small localized reserve is cut back before construction and keeps the lighter tool. |
| report `power-provider` | Ordinary copper-free crank-vs-treadle and settlement treadle-vs-walking-wheel investment choices from disclosed consumer projects rather than arbitrary charge counts. Primitive projects are finite stone-crusher feed orders and settlement projects are finite powered-sawmill lumber orders; authored specific energy converts those visible projects into the required stored-work horizon before provider choice is frozen. Canonical projections price the complete provider-plus-store package, including construction physiology and carried condition. Matched arms execute construction, one full charge, one real productive discharge, and a second recharge from the carried worn/survival state. Longer project lifecycle remains explicitly projected rather than simulated through fake discharge. |
