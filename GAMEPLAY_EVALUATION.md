...
| `progression` | Ordinary coarse-to-fine evidence, sampling-gated mining, primitive crafting/mining/power/processing, fallback versus mechanization, and reinvestment, with maintenance retained as an explicit lifecycle obligation when condition actually demands it. A disclosed finite processing order is priced before construction from production-owned manual-work/power physics plus the complete visible assembly package; mechanization is frozen only when that conservative upper bound beats the hand route, and execution must agree with the projected package. Ordinary dressing can continue through concentration/scavenging and finite-recovery concentrate cleanup into native copper because the current ore model represents liberated copper as an elemental constituent. Separate first-foundry coverage then proves ordinary acquisition and execution from a disclosed material opportunity: the existing timber treadle converts additively into a dual mechanical/electrical dynamo, player work charges a finite electrical buffer, the thermal owner melts and casts 20 g copper scrap, and ordinary cold working returns the cast ingot to reinforcement. That coverage does not claim state continuity from the liberation episode. Its pre-investment decision frame must compare all immediately usable copper routes; for the maintained 20 g reinforcement order, direct native-copper cold working wins and foundry construction is deferred, while the installed-foundry branch separately measures complete scrap recovery against faster cold rework that leaves chips. The remaining copper frontier is industrial foundry scale, not first casting or a missing concentrate-reduction rule. Sampling work precedes hardness classification; validation confirms legality without probing hidden hardness. Forced service used only by an unselected comparison branch is coverage evidence, never evidence that the selected player path maintained equipment. |
| report `primitive-liberation` | Ordinary raw-stone/log acquisition of the maintained primitive processing kit, with a disclosed multi-batch workload required to justify construction before action. The same admitted state then executes the timber-riddle ore route and proves kit payback against canonical manual recovery in attention, body cost, and recovered copper. A separate disclosed first-foundry opportunity exercises ordinary acquisition and recovery mechanics without pretending that its starting inventory was carried from the one-batch liberation state. The report must keep the immediate build decision separate from installed recovery performance: direct native copper may defer foundry construction for a small current order even though an installed foundry can recover scrap more completely. Exploratory worlds may start from disclosed preassembled ordinary kit to vary ore/feed behavior; summary output distinguishes those from the live acquisition witness. |
