...
| Primitive progression | Stone tool shaping/assembly, hand mining, scarce native-copper reinforcement of extraction or stored-work rate, material-backed flywheel storage, hand-crank charging, a player-built primitive crusher, and a player-built rocking separator. Only one reinforcement is directly supplied by the native-copper seam; the other must be recovered from composition-bearing crushed ore, so delegated processing now creates both returned player attention and a concrete material progression path. Upgrade order changes which affordance arrives first, then both branches converge through crusher-to-separator processing. The maintained progression evaluation separately reports automation attention payback and the separator's additional material-progression setup cost. |
