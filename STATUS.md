...
| Prospecting knowledge | Persisted acquired observations with spatial evidence and bounded abundance estimates; deterministic assessment of overlapping, contradictory, and disjoint evidence. Recording requires an opaque prospecting resolution. |
| Mining | Hand mining with real tools, exclusive player labor, throughput/batch/hardness capability, wear, destination reservation, in-flight ownership, and explicit output claim. |
...
| Survival | Metabolic energy, hydration, vitality, Grain/Fruit/Protein nutrition, basal depletion, work exertion, perishable food, preservation, exact selected meals, and finite drinkable water. Vitality recovery is limited by the weakest recent dietary category, making balanced provisioning materially useful without directly punishing narrow diets; the read-only assessment exposes the resulting diet-supported recovery rate. Consumed matter/fluid remain in terminal conservation accounting. |
| Player labor | One exclusive `PlayerWorkState` across manual crafting, mining, and direct player power. Admission requires sufficient projected metabolic energy and hydration. |
...
| Ore processing | Crushing, grinding, dry screening, and selective oversize regrinding with exact mass/composition/temperature preservation, particle-state rules, finite work energy, equipment limits, duration, and wear. No concentration or composition change. |
...
| Primitive progression | Stone tool shaping/assembly, hand mining, sequential native-copper reinforcement of extraction and stored-work rate, material-backed flywheel storage, hand-crank charging, and a player-built primitive crusher that can work while player labor is used elsewhere. Upgrade order can change which affordance arrives first, while additional finite copper can later converge both capabilities. The maintained progression evaluation now measures repeated-use attention payback rather than treating one autonomous batch as sufficient proof of mechanization value. Crushed ore currently provides direct attention/delegation value but has no downstream ordinary-play acquisition use until concentration/smelting capability becomes reachable. |
...
| Geological world systems | Regional geological generation, voxel ore topology, physical prospecting actions such as panning, sampling, drilling, assays, or geophysics. |
...
| Thermal/chemical industry | Environmental heat transport, vaporization, combustion/fuels/emissions, mixed/alloy phase behavior, mineral concentration, chemical smelting/reduction, alloying, forging, machining. |
...
| Industrial acquisition | Runtime paths for industrial machine acquisition, industrial power generation, mixed-ore concentration/smelting, and the broader infrastructure needed to reach the bootstrapped industrial harnesses through ordinary play. |
