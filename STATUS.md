...
| Crafting and equipment | Manual shaping/joinery, persistent equipment, wear, and exact-component service/recovery. Worn disassembly scraps only the wear component; other components stay intact. Boards retain a hand fallback; authored adzes and copper upgrades trade throughput, yield recovery, and attention. The saw bench requires an authored timber frame plus a replaceable copper blade and improves board recovery. Reinforcement extends extraction, sampling, woodworking, power, and processing equipment through authored routes. |
| Primitive power | Manual generation feeds lossy flywheels. Authored crank and treadle providers trade build mass, metabolic efficiency, and output power. Authored timber, stone, copper-banded, and paired stores trade stored-work capacity, input limits, and build mass. All remain short work buffers, not batteries. |
| Primitive processing | Portable crusher, quern, riddle/screens, and separator provide crush -> grind -> size -> regrind -> concentrate. Authored copper upgrades improve batch mass and throughput while preserving service components. First-pass tailings retain copper; finer regrinding enables one lower-grade scavenger pass at added work, energy, and wear, producing terminal exhausted tailings. |
