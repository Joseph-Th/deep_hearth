...
`local clues -> coarse-to-fine prospecting -> physical sampling -> evidence-gated mining -> hand/primitive native-copper recovery -> copper upgrades -> settlement mechanization -> reinforcement/recovery`
...
| Crafting and equipment | Manual shaping/joinery, persistent wear, and exact-component service/recovery. Boards keep a hand fallback; adzes and the copper-bladed frame saw trade attention and recovery. The treadle hammer accelerates copper reinforcement, scrap rework, and saw-blade cold-working. Copper-era upgrades convert the frame saw into a flywheel sash sawmill and the treadle hammer into a flywheel helve hammer. Both retain manual fallback and maintenance identity; powered use consumes finite mechanical work, applies wear, preserves the same material transform, and frees player attention during execution. Copper screen perforation remains hand-only because no punch/die is authored. |
| Primitive power | Manual generation feeds lossy flywheels. Crank, treadle, and walking-wheel providers trade build mass, metabolic efficiency, wear, and output. Timber, stone, copper-banded, paired, and settlement-bank stores trade capacity, input limits, drag, and build mass. The same finite stores feed ordinary sawmill and helve-hammer work directly; no shaft/belt network is implied. The 5 kJ bank covers the largest authored settlement comminution batch. All remain work buffers, not batteries. |
