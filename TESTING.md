...
| `cargo test-gameplay` | Replay contracts, maintained anchors, a bounded fresh workshop sample, progression, and capability probes | Dedicated integration target with `test-gameplay`; library unit-test bodies are not compiled |
...
priorities vary independently of world physics within bounded legal choices: reserve conservation,
projected machine condition, or completion time; preventive service at warning versus service only when
critical; and preserving structural margin versus relocating only when failure forces the issue. Safety,
...
sized from the load it actually carries so both are initially legal, while background stored cargo and
delivery mass scale from current equipment/material quantities. The delivery tick is selected inside a
work horizon derived from an actually resolved batch duration. The support generator deliberately spans
a broad ordinary utilization range rather than targeting authored failure thresholds, so structural
outcomes can change naturally as game balance evolves. Workshop ore
...
primitive progression probe, and two separately named capability-probe tests. The split makes each
expensive behavior slice directly targetable without
...
systems. `EXPERIENCE` lines include final condition and remaining mechanical/maintenance reserves, while
the systems summary counts throughput, energy-delivery, and balanced bottlenecks per completed batch
rather than reducing them to one boolean per scenario. The content summary distinguishes
...
- `DEEP_HEARTH_GAMEPLAY_VARIATION_SEED`: fixes the organic world/scenario root to one exact decimal or
  hexadecimal value; when omitted, a fresh root is generated for the run;
...
Seed lists fail on an empty or malformed entry rather than silently dropping it. Hard failures remain
...
aggregate outcome coverage. Scenario-only summaries explicitly mark the progression/ore/foundry probes
as not run rather than implying results from separate targets.
