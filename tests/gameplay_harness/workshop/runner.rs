...
pub(super) fn run_gameplay_harness(mode: ScenarioPlanMode) {
    ...
    std::println!(
        "PLAYABILITY runtime-actions-after-controlled-bootstrap=[survival-provisioning,manual-shaping,equipment-assembly+upgrade,hand-mining,material-backed-flywheel-construction,survival-costed-manual-power,primitive-autonomous-crushing] primitive-crusher-utility=[attention:playable progression-leverage:second-upgrade-work material-progression:capability-only-downstream] bootstrap-assumptions=[starting-authored-food+drink+storage-profile,raw-gathered-matter,preauthorized-mining-site-identities] discovery=not-implemented capability-only=[industrial-workshop,industrial-ore-preparation,pure-copper-foundry] missing-bridge=[industrial-acquisition,industrial-power-generation,mixed-ore-concentration/smelting]"
    );
    std::println!(
        "PLAYER LOOP runtime-after-bootstrap=[survive->shape-tools->mine->choose-first-copper-affordance:[hard-material-access|stored-work-rate]->mechanize->use-autonomous-time-for-second-upgrade->converge] capability-workshop=[site-machine->process-total-mass->adapt-batch-to-condition+stored-work->choose-power->hand-charge-or-protect-survival->react-to-world-load->maintain-or-relocate->iterate] utility=[material-access,player-attention,survival-reserve,machine-condition,structural-margin,stored-work,time]"
    );
    std::println!(
        "CORE FANTASY REVIEW reachable=[survive->craft-tools->extract->sequence-competing-investments->store-work->delegate-repetition->spend-returned-attention-on-progression] captured-by=[survival-provisioning,primitive-upgrade-sequencing+convergence,manual-power,autonomous-crushing] industrial-depth=[workshop-operation,ore-preparation,pure-copper-foundry] industrial-reachability=capability-only boundary=[crushed-ore-downstream-material-progression,industrial-acquisition,industrial-power-generation,mixed-ore-concentration/smelting]"
    );
    println!(
