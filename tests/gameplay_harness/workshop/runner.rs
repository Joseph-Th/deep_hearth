...
pub(super) fn run_gameplay_harness(mode: ScenarioPlanMode) {
    ...
    std::println!(
        "PLAYER LOOP runtime-after-bootstrap=[survive->shape-tools->mine->choose-first-copper-affordance:[hard-material-access|stored-work-rate]->mechanize->mine-more-while-crushing->separate-crushed-ore->forge-second-upgrade->converge] capability-workshop=[site-machine->process-total-mass->adapt-batch-to-condition+stored-work->choose-power->hand-charge-or-protect-survival->react-to-world-load->maintain-or-relocate->iterate] utility=[material-access,player-attention,processed-material,survival-reserve,machine-condition,structural-margin,stored-work,time]"
    );
    std::println!(
        "CORE FANTASY REVIEW reachable=[survive->craft-tools->extract->sequence-competing-investments->store-work->delegate-repetition->convert-processed-matter-into-next-capability] captured-by=[survival-provisioning,primitive-upgrade-sequencing+convergence,manual-power,autonomous-crushing,composition-derived-native-copper-separation] industrial-depth=[workshop-operation,ore-preparation,pure-copper-foundry] industrial-reachability=capability-only boundary=[industrial-acquisition,industrial-power-generation,generalized-mixed-ore-concentration/smelting]"
    );
    println!(
