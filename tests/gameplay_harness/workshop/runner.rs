...
pub(super) fn run_gameplay_harness(mode: ScenarioPlanMode) {
    ...
    std::println!(
        "EVIDENCE SCOPE reachable-runtime-after-controlled-bootstrap=[survival,field-prospecting,hand-mining,manual-crafting,primitive-power,primitive-processing] controlled-capability-probes=[industrial-workshop,industrial-ore-preparation,pure-copper-foundry] bootstrap=[starting-food+drink+raw-gathered-matter+visible-regional-geological-clue-zones+local-follow-up-regions] registry-route-discovery=CONTENT-PROCESS-ROUTES global-reachability-boundary=STATUS.md"
    );
    if verbose {
        std::println!(
            "PLAYABILITY EVIDENCE runtime-actions-after-controlled-bootstrap=[pressure-sensitive-survival-response+activity-dependent-work-pressure+provisioning,regional-reconnaissance->evidence-prioritized-local-prospecting->evidence-gated-mining,deferred-evidence-refinement-when-current-options-fail,manual-shaping,equipment-assembly+upgrade,hand-mining,material-backed-flywheel-construction,survival-costed-manual-power,primitive-autonomous-crushing,primitive-constituent-separation] bootstrap-assumptions=[starting-authored-food+drink+storage-profile,raw-gathered-matter,visible-regional-geological-clue-zones+local-follow-up-regions] actor-hidden=[deposit-identity,future-controlled-event] note=claims-are-limited-to-executed-probes"
        );
        std::println!(
            "PLAYER LOOP runtime-after-bootstrap=[recon-regional-clue-zones->prioritize-local-inspection->act-on-resolved-evidence+defer-uncertain-clues->respond-to-hunger/thirst+choose-work-that-shifts-body-pressure+shape-tools->mine->learn-form+composition-from-extraction->revisit-uncertainty-when-direct-supply-fails->choose-processing-feed-from-observed-matter->choose-first-copper-affordance:[hard-material-access|stored-work-rate]->mechanize->mine-more-while-crushing->repay-automation-attention-before-wear-endpoint->separate-crushed-ore->forge-second-upgrade->converge] capability-workshop=[site-machine->process-total-mass->adapt-batch-to-condition+stored-work->choose-power->hand-charge-or-protect-survival->react-to-world-load->maintain-or-relocate->iterate] utility=[information,material-access,player-attention,processed-material,survival-reserve,machine-condition,structural-margin,stored-work,time]"
        );
        std::println!(
