...
#[path = "runner/maintenance_policy.rs"]
mod maintenance_policy;
use maintenance_policy::*;
...
fn prepare_episode(registries: &Registries, variation: ScenarioVariation) -> WorkshopEpisode {
    ...
    assert!(
        compact_assessment
            .is_within_utilization_limit(variation.structure.compact_target_utilization_ppm),
        "gameplay compact-support fixture drifted above its production structural-utilization target"
    );
    assert!(
        reinforced_assessment
            .is_within_utilization_limit(variation.structure.reinforced_target_utilization_ppm),
...
pub(super) fn run_gameplay_harness(mode: ScenarioPlanMode) {
    ...
    #[cfg(not(test))]
    {
        ...
        std::println!(
            "EVIDENCE CONTRACT runtime-experience-after-disclosed-bootstrap=[survival,primitive-progression,woodworking,fieldwork,power-provider] controlled-capability-probes=[industrial-workshop,industrial-ore-preparation,pure-copper-foundry] setup-shortcuts=disclosed-and-fixture-guarded actor-observation=runtime-public-state-only catalog=registry-derived authored-edges=not-end-to-end-proof global-reachability-authority=STATUS.md"
        );
        if verbose {
            std::println!(
