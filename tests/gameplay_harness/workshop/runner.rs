...
pub(super) fn run_gameplay_harness(mode: ScenarioPlanMode) {
    ...
    std::println!(
        "PLAYABILITY runtime-actions-after-controlled-bootstrap=[survival-provisioning,surface-prospecting->evidence-refinement->opaque-mining-target,manual-shaping,equipment-assembly+upgrade,hand-mining,material-backed-flywheel-construction,survival-costed-manual-power,primitive-autonomous-crushing,primitive-constituent-separation] primitive-processing-utility=[attention:playable progression-leverage:second-upgrade-work material-progression:playable-separated-copper] bootstrap-assumptions=[starting-authored-food+drink+storage-profile,raw-gathered-matter,visible-local-geological-clue-regions] discovery=[quick-local-inspection:playable detailed-local-refinement:playable ambiguity-recovery:playable evidence-to-extraction:playable hidden-deposit-id:unavailable-to-actor] capability-only=[industrial-workshop,industrial-ore-preparation,pure-copper-foundry] missing-bridge=[world-scale-clue-discovery,industrial-acquisition,industrial-power-generation,generalized-mixed-ore-concentration/smelting]"
    );
    std::println!(
        "PLAYER LOOP runtime-after-bootstrap=[inspect-local-clues->refine-if-ambiguous->resolve-target->survive+shape-tools->mine->choose-first-copper-affordance:[hard-material-access|stored-work-rate]->mechanize->mine-more-while-crushing->separate-crushed-ore->forge-second-upgrade->converge] capability-workshop=[site-machine->process-total-mass->adapt-batch-to-condition+stored-work->choose-power->hand-charge-or-protect-survival->react-to-world-load->maintain-or-relocate->iterate] utility=[information,material-access,player-attention,processed-material,survival-reserve,machine-condition,structural-margin,stored-work,time]"
    );
    std::println!(
        "CORE FANTASY REVIEW reachable=[inspect-world->narrow-uncertainty->survive->craft-tools->extract->sequence-competing-investments->store-work->delegate-repetition->convert-processed-matter-into-next-capability] captured-by=[survival-costed-two-resolution-local-prospecting,geological-evidence-gated-extraction,survival-provisioning,primitive-upgrade-sequencing+convergence,manual-power,autonomous-crushing,composition-derived-native-copper-separation] industrial-depth=[workshop-operation,ore-preparation,pure-copper-foundry] industrial-reachability=capability-only boundary=[world-scale-clue-discovery,material-sampling/drilling/assays/geophysics,industrial-acquisition,industrial-power-generation,generalized-mixed-ore-concentration/smelting]"
    );
    println!(
