...
pub(super) fn run_gameplay_harness(mode: ScenarioPlanMode) {
    ...
    std::println!(
        "PLAYABILITY runtime-actions-after-controlled-bootstrap=[survival-provisioning,surface-prospecting->evidence-gated-mining,deferred-evidence-refinement-when-current-options-fail,manual-shaping,equipment-assembly+upgrade,hand-mining,material-backed-flywheel-construction,survival-costed-manual-power,primitive-autonomous-crushing,primitive-constituent-separation] primitive-processing-utility=[attention:playable progression-leverage:second-upgrade-work material-progression:playable-separated-copper] bootstrap-assumptions=[starting-authored-food+drink+storage-profile,raw-gathered-matter,visible-local-geological-clue-regions] discovery=[quick-local-inspection:playable detailed-local-refinement:playable constraint-triggered-ambiguity-recovery:playable evidence-to-extraction:playable extracted-form+composition-observation:playable hidden-deposit-id:unavailable-to-actor] capability-only=[industrial-workshop,industrial-ore-preparation,pure-copper-foundry] missing-bridge=[world-scale-clue-discovery,industrial-acquisition,industrial-power-generation,concentrate-reduction/smelting]"
    );
    std::println!(
        "PLAYER LOOP runtime-after-bootstrap=[inspect-local-clues->act-on-resolved-evidence+defer-uncertain-clues->survive+shape-tools->mine->learn-form+composition-from-extraction->revisit-uncertainty-when-direct-supply-fails->choose-processing-feed-from-observed-matter->choose-first-copper-affordance:[hard-material-access|stored-work-rate]->mechanize->mine-more-while-crushing->separate-crushed-ore->forge-second-upgrade->converge] capability-workshop=[site-machine->process-total-mass->adapt-batch-to-condition+stored-work->choose-power->hand-charge-or-protect-survival->react-to-world-load->maintain-or-relocate->iterate] utility=[information,material-access,player-attention,processed-material,survival-reserve,machine-condition,structural-margin,stored-work,time]"
    );
    std::println!(
        "CORE FANTASY REVIEW reachable=[inspect-world->infer-affordances->defer-low-value-uncertainty->respond-to-new-constraints-with-better-information->survive->craft-tools->extract->sequence-competing-investments->store-work->delegate-repetition->convert-processed-matter-into-next-capability] captured-by=[survival-costed-local-prospecting,goal-triggered-evidence-refinement,geological-evidence-gated-extraction,extracted-form+composition-feedback,survival-provisioning,primitive-upgrade-sequencing+convergence,manual-power,autonomous-crushing,inventory-derived-native-copper-separation] industrial-depth=[workshop-operation,ore-preparation,pure-copper-foundry] industrial-reachability=capability-only boundary=[world-scale-clue-discovery,material-sampling/drilling/assays/geophysics,industrial-acquisition,industrial-power-generation,concentrate-reduction/smelting]"
    );
    println!(
