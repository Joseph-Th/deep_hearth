...
pub(super) fn run_gameplay_harness(mode: ScenarioPlanMode) {
    ...
    std::println!(
        "CORE FANTASY REVIEW reachable=[inspect-world->infer-affordances->defer-low-value-uncertainty->respond-to-new-constraints-with-more-precise-information->survive->craft-tools->extract->sequence-competing-investments->store-work->delegate-repetition->reinvest-returned-attention->convert-processed-matter-into-next-capability] captured-by=[survival-costed-local-prospecting,goal-triggered-evidence-refinement+negative-information,geological-evidence-gated-extraction,extracted-form+composition-feedback,pressure-sensitive-survival-provisioning+activity-dependent-work-pressure,material-primitive-upgrade-windows+convergence,manual-power,condition-limited-autonomous-crushing+bounded-attention-payback,inventory-derived-native-copper-separation] industrial-depth=[workshop-operation,full-batch-ore-preparation,pure-copper-foundry] industrial-reachability=capability-only boundary=[world-scale-clue-discovery+spatially-coarse-to-local-refinement,material-sampling/drilling/assays/geophysics,industrial-acquisition,industrial-power-generation,concentrate-reduction/smelting]"
    );
    println!(
