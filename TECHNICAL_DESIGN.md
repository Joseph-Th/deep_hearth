...
| --- | --- | --- |
| Hidden geology -> acquired knowledge | `validate_start_field_prospecting` -> simulation tick | `PlayerWorkState` holds exclusive prospecting labor; completion records one bounded `GeologicalObservationRecord` in `GeologicalKnowledgeState`. All methods record bounded abundance. Authored physical-sampling methods may additionally record a conservative excavation-hardness band only when their material evidence has a positive lower bound; uncertain/negative evidence therefore cannot reveal hidden presence through hardness metadata. `TickOutcome::field_prospecting()` exposes the actor-safe observation identity and scope without deposit identity. |
