...
| --- | --- | --- |
| Thermal adaptive batch sizing requires repeated semantic probing | `tests/gameplay_harness/foundry_probe.rs` binary-searches melt and cast mass by repeatedly resolving nearby requests and classifying capacity, finite-energy, and condition-lifetime failures. | Treat this as a candidate for a reusable thermal feasible-envelope/bottleneck projection derived from the same physical rules. It is planning-surface debt, not a legality or conservation defect. |
| Powered ore planning repeats the same scale constraints across several operations | workshop crushing binary-searches resolvable and maintenance-safe batch mass; ore capability probes separately classify finite energy, batch capacity, and condition lifetime for crushing, grinding, screening, and concentration. These operations already share authored powered-profile concepts. | Prefer domain-level ore planning bounds/constraints derived from the shared physical profiles where semantics truly match. Do not flatten unrelated thermal/mining/equipment failures into one global blocker type. |
| Production completion is already receipt-shaped but one shared harness helper ignores it | `TickOutcome::production_completions()` identifies completed jobs, while `tests/gameplay_harness/production_support.rs` instead rereads `ProductionState` after every tick to notice that the job disappeared. | Normalize callers to consume an existing typed receipt before enlarging production APIs. A redundant state rescan is caller debt when the canonical outcome already answers the question. |
...
| Prospecting gameplay currently verifies completion by state counting | progression starts prospecting, discards the token's exposed work schedule and every tick outcome, advances by the authored definition duration, then proves success by comparing observation counts. | A future canonical batched-step helper should make consuming `FieldProspectingOutcome` cheaper. This is principally caller/continuation ergonomics, not evidence that prospecting commit itself needs a new outcome. |
| Authored acquisition topology is reconstructed in several harness paths | `tests/gameplay_harness/report.rs` classifies every process by scanning resolver registries and matching provider/energy definitions; `preservation_route.rs` recursively scans manual-craft outputs to discover construction routes; `progression_probe.rs` independently scans outputs to select component-production routes; catalog tests repeat producer/recovery scans. | High-leverage candidate: a registry-derived, typed possibility topology or domain reverse indexes shared by legitimate callers. Keep actor cost preferences and tie-breaking outside it, and keep ordinary/current reachability separate. |
| Process catalog classification repeats cross-registry admission semantics | `report.rs::process_resolver_and_energy` probes crafting, ore-processing, and thermal registries to rediscover the unique execution family that `Registries::new` already enforces, then independently matches nominal equipment capabilities and energy-store carrier direction. | Prefer a cross-registry derived process-topology projection built at registry assembly. Keep runtime provider condition/support in `resolve_equipment_provider`; do not conflate definition discovery with runtime provider selection. |
| Direct-route terminology is weaker than its surface wording can suggest | equipment `has_runtime_acquisition_route()` and energy `has_runtime_assembly_route()` classify local authored fields; reports explicitly warn that these declarations are not end-to-end reachability, while separate content tests recursively prove selected primitive roots. | Preserve the useful local predicate but standardize edge/path/reachability/opportunity terminology. Rename only when the owning API is otherwise touched; do not create compatibility aliases. |
...
| Deterministic lot choice remains harness policy | `tests/gameplay_harness/production_support.rs` chooses exact lots from observable stockpile order to satisfy an actor-selected mass. | Do not move this into production merely for reuse. The inventory owner supplies authoritative lots; tie-breaking among otherwise legal observable inputs belongs to actor policy unless product semantics require a canonical choice. |
...
4. **Expose proven monotonic envelopes.** Replace repeated request probing with domain-specific feasible bounds
   where several callers vary one quantity and production already derives the same limiting dimensions. Thermal
   batch sizing and powered ore batch planning are current evidence.
